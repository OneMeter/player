...
#import "ZJC_LEVEL1_TableViewCell.h"

@implementation ZJC_LEVEL1_TableViewCell
...
- (void)configSelf{
    ...
    // 标题
    self.myTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake([ZJCFrameControl GetControl_X:80], [ZJCFrameControl GetControl_Y:35], [ZJCFrameControl GetControl_weight:1080-60], [ZJCFrameControl GetControl_height:40])];
    self.myTitleLabel.textColor = ZJCGetColorWith(100, 100, 100, 1);
    self.myTitleLabel.font = [UIFont systemFontOfSize:fontsize];
    [self.contentView addSubview:self.myTitleLabel];
    
    // 选择按钮
    self.selectButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.selectButton.frame = CGRectMake([ZJCFrameControl GetControl_X:1080 - 10 - 88 - 30], 0, [ZJCFrameControl GetControl_weight:88 + 10 + 30], [ZJCFrameControl GetControl_Y:120]);
    ...
    // 选中图片
    self.selectImageView = [[UIImageView alloc] initWithFrame:CGRectMake([ZJCFrameControl GetControl_X:30], [ZJCFrameControl GetControl_Y:35], [ZJCFrameControl GetControl_weight:44], [ZJCFrameControl GetControl_weight:44])];
...
- (void)selectButtonClicked:(UIButton *)button{
    ...
    // 回调
    if ([self cell1SelectBlock]) {
        self.cell1SelectBlock(self,button);
    }
}
...
- (void)drawRect:(CGRect)rect{
    ...
    CGContextMoveToPoint(context, [ZJCFrameControl GetControl_X:60], self.bounds.size.height);  //起点坐标
